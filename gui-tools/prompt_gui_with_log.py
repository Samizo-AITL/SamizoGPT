...
LOG_PATH = os.path.join(os.path.dirname(__file__), "prompt_log.md")
...
class PromptApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame, text="コピー", command=self.copy_to_clipboard).grid(row=0, column=0, padx=5)
        ttk.Button(frame, text="保存", command=self.save_prompt).grid(row=0, column=1, padx=5)
    ...
    def save_prompt(self):
        text = self.output_text.get("1.0", "end").strip()
        if not text:
            messagebox.showwarning("未出力", "保存するプロンプトがありません。")
            return
        try:
            with open(LOG_PATH, "a", encoding="utf-8") as f:
                f.write(f"## {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(text + "\n\n")
            messagebox.showinfo("保存完了", f"prompt_log.md に保存しました。")
        except Exception as e:
            messagebox.showerror("保存エラー", f"ログ保存に失敗しました：\n{e}")
